export function renderContactVerifyEmail({
  code,
  expiresInMinutes,
  listingTitle,
}: {
  ...
  const subject = `Confirm your email to get notified about "${listingTitle}"`;
  const text = [
    `Confirm your email to get notified when "${listingTitle}" closes`,
    "",
    code,
    "",
    ...
    <div style="padding:32px 16px">
      <div style="max-width:520px;margin:0 auto;border:1px solid #2b3038;border-radius:24px;overflow:hidden;background:#14181f">
        ...
        <div style="padding:34px">
          ...
          <p style="margin:0;color:#a1a1aa;font-size:15px;line-height:1.7">Enter this code to get notified when <strong style="color:#ffffff">${listingTitle}</strong> closes.</p>
